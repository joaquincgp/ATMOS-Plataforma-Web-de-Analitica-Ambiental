...
export function ResetPassword({ initialToken, onReset, onBackToLogin }: ResetPasswordProps) {
  ...
  const submit = form.handleSubmit(async (values) => {
    ...
    if (!token) {
      setError('El enlace de recuperación no es válido o está incompleto. Solicita uno nuevo.');
      return;
    }

    try {
      await onReset({ token, new_password: values.newPassword });
    ...
    <div className="min-h-screen bg-[#F9FBFC] flex items-center justify-center p-4">
      <Card className="w-full max-w-md bg-white shadow-lg">
        <CardHeader className="space-y-2 text-center">
          ...
          <CardDescription>Enter your new password to complete account recovery.</CardDescription>
        </CardHeader>
        <CardContent>
